    ...
    <ScreenBackground>
        ...
        <View className="flex-row items-center gap-2 px-5 pb-3 border-b border-white/10">
          ...
          >
            <Text className="text-xs font-sans-semibold text-white flex-shrink" numberOfLines={1}>
              {selectedCategory ? selectedCategory.name : t('courses.category')}
            </Text>
          ...
          >
            <Text className="text-xs font-sans-semibold text-white flex-shrink" numberOfLines={1}>
              {selectedLevel ? levels.find((l) => l.value === selectedLevel)?.label : t('courses.level')}
            </Text>
            <ChevronDown size={14} color="rgba(255,255,255,0.70)" />
          </TouchableOpacity>
        </View>
            ...
            ListEmptyComponent={
              <View className="items-center py-8">
                ...
                <Text className="text-base font-sans-semibold text-white/80 mt-2">
                  {search.trim() ? t('courses.noResults') : t('courses.noCourses')}
                </Text>
                <Text className="text-sm text-white/50 mt-1">
                  {search.trim() ? t('courses.tryAnotherKeyword') : t('courses.noCoursesYet')}
                </Text>
              </View>
            }
        ...
        >
          ...
          >
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-lg font-sans-bold text-white">{t('courses.selectCategory')}</Text>
              <TouchableOpacity
                onPress={() => setCategoryModalVisible(false)}
            ...
            <FlatList
              ...
              ListHeaderComponent={
                ...
                >
                  <Text className="text-sm font-sans-semibold text-white">{t('courses.allCategories')}</Text>
      ...
      >
        ...
        >
          ...
          >
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-lg font-sans-bold text-white">{t('courses.selectLevel')}</Text>
              <TouchableOpacity
                onPress={() => setLevelModalVisible(false)}
                className="w-8 h-8 rounded-full bg-white/10 items-center justify-center"
              >
            ...
            <FlatList
              ...
              ListHeaderComponent={
                <TouchableOpacity
                  ...
                  className="flex-row items-center justify-between py-3 border-b border-white/10"
                >
                  <Text className="text-sm font-sans-semibold text-white">{t('courses.allLevels')}</Text>
