...
export const VideoPlayer = forwardRef<VideoPlayerHandle, VideoPlayerProps>(
  ({ url, onEnd, onError, onTimeUpdate }, ref) => {
    const videoRef = useRef<HTMLVideoElement | null>(null);

    // `onError`/`onTimeUpdate` are inline functions from the caller and get a new
    // identity on every render. They're only read inside effect closures below, so keep
    // the latest value in a ref rather than putting it in the effect's dependency array —
    // depending on it directly would tear down and rebuild the whole HLS player (aborting
    // in-flight segment requests) on every unrelated re-render, which is what produced the
    ...
    return (
      <View style={styles.wrap}>
        <video
          ...
          onEnded={onEnd}
          onError={() => onError?.()}
          onTimeUpdate={(e) => onTimeUpdateRef.current?.(e.currentTarget.currentTime)}
        />
      </View>
    );
  }
